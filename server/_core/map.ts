 ...
 * to Google Maps APIs. In Manus mode requests go through Forge. In external
 * mode requests go directly to Google's web service endpoints.
 */

import { resolveDeployMode, resolveForgeConfig } from "./platform";
...
export function resolveMapsServiceConfig(
  ...
  return {
    ...
    apiKey: env.GOOGLE_MAPS_SERVER_API_KEY?.trim() || "",
  };
}
...
export async function makeRequest<T = unknown>(
  ...
  const { mapsMode, baseUrl, apiKey } = resolveMapsServiceConfig();

  const upstreamPath =
    mapsMode === "forge" ? `/v1/maps/proxy${endpoint}` : endpoint;
